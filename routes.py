...
# Ruta para verificar la existencia de la CURP
@user_routes.route('/users/check-curp', methods=['POST'])
def check_curp():
    data = request.json
    curp = data.get('curp')

    existing_user = TBL_USERS.query.filter_by(curp_users=curp).first()

    if existing_user:
        return jsonify({'exists': True, 'message': 'La CURP ya se encuentra registrada'}), 200
    else:
...
def check_no_control():
    data = request.json
    no_control = data.get('no_control_users')

    existing_user = TBL_USERS.query.filter_by(no_control_users=no_control).first()
...
# Ruta para verificar la existencia del seguro social
@user_routes.route('/users/check-seguro-social', methods=['POST'])
def check_seguro_social():
    data = request.json
    seguro_social = data.get('seguro_social_users')

    existing_user = TBL_USERS.query.filter_by(seguro_social_users=seguro_social).first()

    if existing_user:
        return jsonify({'exists': True, 'message': 'El seguro social ya se encuentra registrado'}), 200
    else:
        return jsonify({'exists': False}), 200
...
def create_user():
    data = request.json
    curp_existente = check_existing_data('curp_users', data['curp_users'])
    no_control_existente = check_existing_data('no_control_users', data['no_control_users'])
    seguro_social_existente = check_existing_data('seguro_social_users', data['seguro_social_users'])

    if curp_existente or no_control_existente or seguro_social_existente:
        return jsonify({'message': 'Error: Datos duplicados', 'curp_existente': curp_existente,
                        'no_control_existente': no_control_existente,
                        'seguro_social_existente': seguro_social_existente}), 400
    ...
    new_user = TBL_USERS(
        ...
        seguro_social_users=data['seguro_social_users'],
        curp_users=data['curp_users'],
        idRol=data['idRol'],
        idSexo=data['idSexo'],
...
# Función para verificar la existencia de datos duplicados
def check_existing_data(field_name, field_value):
    existing_user = TBL_USERS.query.filter_by(**{field_name: field_value}).first()
